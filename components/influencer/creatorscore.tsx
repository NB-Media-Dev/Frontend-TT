...
export default function CreatorScore({
  ...
  return (
    <Cardlayout
      ...
      icon={null}
      isLoading={isLoading}
      skeleton={<ContentSkeleton count={3} height="h-[50px]" width="w-full" />}
      ...
      action={
        <span
          className={`${buttonVariants({ variant: "sucess" })} px-2 text-xs sm:text-sm`}
        >
          {status}
        </span>
      }
    >
      <div className="flex flex-col sm:flex-row md:flex-col lg:flex-row items-center justify-between gap-4 sm:gap-5">
        <div className="relative flex items-center justify-center w-28 h-28 sm:w-32 sm:h-32 md:w-28 md:h-28 lg:w-36 lg:h-36 shrink-0 select-none">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
            <span className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-gray-900 leading-none tracking-tight select-text">
              {score}
            </span>
            <span className="text-[10px] sm:text-xs font-semibold text-gray-400 mt-0.5 sm:mt-1">
              /100
            </span>
          </div>
        </div>
          ...
          {metrics.map((item) => (
            ...
            >
              ...
              <div className="whitespace-nowrap select-none">
                <span className="text-[#10B981] font-black">{item.score}</span>
