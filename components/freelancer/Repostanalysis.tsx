...
export default function Repostanalysis({
  ...
  return (
      ...
      action={
        <div className="relative">
          <button
            ...
            className="bg-[#F0F4F8] hover:bg-[#E4ECF3] active:scale-95 text-gray-700 text-xs  px-3 py-1.5 rounded-full flex items-center gap-1 transition-all cursor-pointer select-none"
          >
          ...
          {isDropdownOpen && (
            <div className="absolute right-0 mt-1 w-36 bg-white border border-gray-100 shadow-lg rounded-2xl py-1.5 z-20 overflow-hidden animate-in fade-in slide-in-from-top-1 select-none">
    ...
    >
      {/* 
        ===================================================================
        CHILDREN SLOT: Numeric statistics and graph grids mount here
        ===================================================================
      */}
      <div>
        {/* Upper Column Pair Aspect: Statistical Information Readouts */}
        <div className="grid grid-cols-2 gap-4 mb-4">
          {/* Box 1: Impressions Analytics Track */}
          <div className="flex flex-col">
            <span className="text-[11px] font-semibold text-gray-400 block mb-0.5">
              Impressions
            </span>
            <div className="text-xl sm:text-2xl font-black text-gray-900 tracking-tight leading-tight select-text">
              {impressions}
            </div>
          ...
          {/* Box 2: Financial Revenue Analytics Track */}
          <div className="flex flex-col">
            <span className="text-[11px] font-semibold text-gray-400 block mb-0.5">
              Revenue
            </span>
        ...
        <div className="bg-[#F0F4F8]/70 border border-gray-100/60 rounded-[28px] p-4 sm:p-5 pt-6 flex flex-col justify-end">
          ...
          <div className="h-28 sm:h-32 flex items-end justify-between gap-1.5 sm:gap-2 px-1">
            {dailyData.map((item, index) => {
              ...
              return (
                ...
                >
                  {/* Absolute Dynamic Popover Tooltip Floating Bubble */}
                  {isHovered && (
                    <div className="absolute -top-7 bg-gray-900 text-white text-[10px] font-bold px-1.5 py-0.5 rounded shadow-sm whitespace-nowrap z-10 animate-in fade-in select-text">
                      {item.value}
                    </div>
                  )}

                  {/* Individual Vector Pillar Node with explicit CVA layout switches */}
                  <div
          ...
          {/* Lower X-Axis Day Indicator Text Labels Container */}
          <div className="flex items-center justify-between gap-1.5 sm:gap-2 px-1 mt-3">
            {dailyData.map((item) => (
              <span
                key={item.day}
                className={`flex-1 text-center text-[9px] sm:text-[10px] font-bold uppercase tracking-wider ${
                  item.isPeak ? "text-[#FF6B35]" : "text-gray-400"
                }`}
              >
                {item.day}
              </span>
